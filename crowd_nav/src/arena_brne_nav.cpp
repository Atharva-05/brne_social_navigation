...
class PathPlan{
  public:
    ...
    : goal_set{false}, walls_generated{false}
    {
      ...
      // ros::param::param("odom_topic", _odom_topic, "/jackal/odom");
      // ros::param::param("ped_pos_topic", _pedestrain_position, "/pedsim_agents/semantic/pedestrian");
      _odom_topic = "/jackal/odom";
      _pedestrain_position_topic = "/pedsim_agents/semantic/pedestrian";
      ...
      goal_sub_ = nh->subscribe("goal_pose", 10, &PathPlan::goal_cb, this);
      // gazebo_model_states_sub_ = nh->subscribe("gazebo/model_states_throttle", 33, &PathPlan::gazebo_model_states_cb, this);
      pedestrians_sub_ = nh->subscribe(_pedestrain_position_topic, 33, &PathPlan::pedestrians_cb, this);
  ...
  private:
    ...
    std::string _odom_topic, _pedestrain_position_topic;
    ...
    {
      ROS_INFO_STREAM("Goal Received: " << msg.pose.position.x << ", " << msg.pose.position.y);
      goal_set = true;
      goal = msg;
      check_goal();
      trial_start = ros::Time::now();
    ...
    void ped_callback(const pedsim_msgs::SemanticData::ConstPtr &msg){

      // pedsim_msgs::SemanticDatum[] peds_list = msg->points;
      ...
      if(n_peds < 1){
        ...
        return;
          ...
          // Pedestrian(s) found
          ROS_INFO_THROTTLE(1, "%d pedestrians detected.", n_peds);
          crowd_nav_interfaces::PedestrianArray ped_array;
          ped_array.header.frame_id = "odom";
    ...
    void gazebo_model_states_cb(const gazebo_msgs::ModelStates::ConstPtr& msg)
    {
      std::vector<std::string> model_names_list = msg->name;
      int n_peds = model_names_list.size() - 2;
      auto it = std::find(model_names_list.begin(), model_names_list.end(), "go1_gazebo");
      size_t robot_index = std::distance(model_names_list.begin(), it);
      geometry_msgs::Pose robot_ph = msg->pose[robot_index];

      if (goal_set){
        trial_path_length += dist(robot_pose.x, robot_pose.y, 
                                  robot_ph.position.x, robot_ph.position.y);
      }
      // get the angle from the quaternion
      tf2::Quaternion q(robot_ph.orientation.x, robot_ph.orientation.y,
        robot_ph.orientation.z, robot_ph.orientation.w);
      tf2::Matrix3x3 m(q);
      double roll, pitch, yaw;
      m.getRPY(roll, pitch, yaw);
      robot_pose.x = robot_ph.position.x;
      robot_pose.y = robot_ph.position.y;
      robot_pose.theta = yaw;
      if (goal_set) {
        check_goal();
      }
      ...
      for (int i = 1; i <= n_peds; ++i) {
          ...
          // Find the index of the pedestrian in the model_names_list
          auto ped_index_it = std::find(model_names_list.begin(), model_names_list.end(), "actor" + std::to_string(i));
          
          // Check if the pedestrian was found in the list
          if (ped_index_it != model_names_list.end()) {
              size_t ped_index = std::distance(model_names_list.begin(), ped_index_it);
              ped_i.pose = msg->pose[ped_index];
              ped_array.pedestrians.push_back(ped_i);
          } else {
              // Handle case where the pedestrian is not found
              ROS_WARN("Pedestrian actor%d not found in the model states.", i);
          }}
